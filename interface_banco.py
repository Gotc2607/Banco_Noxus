...
class menu_interface(banco_de_dados2):
    ...
    #tela principal para os usuarios já cadastrados
    def tela_principal(self, usuario, senha, saldo):
        # Tela principal com opções (Depósito, Saque, etc.)
        self.limpar_tela()
        ...
        tk.Button(self.janela, text='Depositar', command=self.tela_deposito, fg='red', bg='gray').pack(pady=5) #botão de deposito

        tk.Button(self.janela, text='Sacar', command=self.tela_saque, fg='red', bg='gray').pack(pady=5)# botão de saque
    ...
    def tela_login(self):
        ...
        tk.Button(self.janela, text='Login', command=self.fazer_login, fg='red', bg='grey').grid(row=5, column=1,pady=3) #botão para fazer login
        tk.Button(self.janela, text='Cadastrar', command= self.tela_novo_usuario,fg='red', bg='grey').grid(row=5, column=0,pady=3) #botão parase cadastrar
        tk.Button(self.janela, text='Sair', command=self.finalizar, fg='red', bg='grey').grid(row=6, column=0, columnspan=2, pady=5)

    def fazer_login(self): # metodo intermediario para receber os valores de usuario e senha
    # Obtém os valores dos campos de entrada
        usuario = self.nome_usuario.get()
        senha = self.senha_usuario.get()

    # Chama o método verificar_usuario e trata o resultado
        if self.verificar_usuario(usuario, senha):
            ...
            self.tela_principal(usuario, senha, saldo)  # Navega para a tela do usuário
        else:
    ...
    def tela_novo_usuario(self):
        ...
        self.nome_usuario.pack(pady=10) #area para colocar o nome do novo usuario
        ...
        def realizar_deposito():
            try:
                ...
                if sucesso:
                    ...
                    self.tela_principal(self.usuario_atual, self.senha_atual, self.ver_saldo(self.senha_atual))  # Volta para a tela principal
                else:
